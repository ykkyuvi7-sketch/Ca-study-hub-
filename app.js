const app = {
    ...
    renderHome() {
        ...
        cards += `</div>`;
        return cards + this.renderContinueWatching();
    },
    ...
    renderChapter() {
        ...
        return lectures.map((l) => {
            ...
            const typeLabel = l.playlistId ? "📁 Playlist" : "🎥 Video";
            return `
                <div class="list-item" onclick="app.navigateTo('lecture', {lectureId: '${l.id}'})">
                    <div class="list-item-info">
                        <strong>${l.title}</strong>
                        <span>${typeLabel} • ${userData.completed ? '✅ Done' : '🕒 Pending'}</span>
                    </div>
                </div>
            `;
        }).join('');
    ...
    renderLectureView() {
        ...
        // Construct the IFrame Source
        // If it's a playlist, use the /videoseries endpoint
        const embedUrl = lecture.playlistId 
            ? `https://www.youtube-nocookie.com/embed/videoseries?list=${lecture.playlistId}`
        ...
        return `
            ...
            <div class="lecture-content">
                ...
                <textarea id="note-area" placeholder="Add your study notes here..." onblur="app.saveNotes('${lecture.id}')">${userData.notes}</textarea>
    ...
    renderSearchUI() {
        return `
            <div class="search-container">
                <input type="text" id="search-input" autofocus placeholder="Search videos or chapters..." oninput="app.handleSearch(this.value)">
    ...
    renderContinueWatching() {
        const uncompleted = lecturesData.filter(l => !this.getUserData(l.id).completed).slice(0, 2);
        if (uncompleted.length === 0) return "";

        return `
            <div style="padding: 0 1rem;"><h3>Up Next</h3></div>
            ${uncompleted.map(l => `
                <div class="list-item" onclick="app.navigateTo('lecture', {lectureId: '${l.id}'})">
                    <div class="list-item-info">
                        <strong>${l.title}</strong>
